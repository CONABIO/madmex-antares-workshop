...
then
  ...
  sudo docker run --rm -v $7/$newdir:/data -w=/data -e terra_cmg=$terra_cmg -e terra_cma=$terra_cma -e aqua_cma=$aqua_cma -e aqua_cmg=$aqua_cmg madmex/ledaps-landsat8  /usr/local/espa-tools/bin/combine_l8_aux_data --terra_cmg=$terra_cmg --terra_cma=$terra_cma --aqua_cmg=$aqua_cmg --aqua_cma=$aqua_cma --output_dir=/data
...
sudo docker run --rm -v $7/$newdir:/data -w=/data -e metadata=$metadata -e metadataxml=$metadataxml madmex/ledaps-landsat8 /usr/local/espa-tools/bin/convert_lpgs_to_espa --mtl=$metadata --xml=$metadataxml
...
sudo docker run --rm -v $7/$newdir:/data -w=/data -e LEDAPS_AUX_DIR=/data -e anc=$anc -e metadataxml=$metadataxml madmex/ledaps-landsat8 /usr/local/espa-tools/bin/lasrc --xml=$metadataxml --aux=$anc --verbose --write_toa
sudo docker run --rm -v $7/$newdir:/data -w=/data -e newdir=$newdir -e metadataxml=$metadataxml madmex/ledaps-landsat8 /usr/local/espa-tools/bin/convert_espa_to_hdf --xml=$metadataxml --hdf=lndsr.$(echo $newdir).hdf --del_src_files
